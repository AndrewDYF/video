...
class VideoController extends Controller {
  ...
  async create() {
    ...
    try {
        ...
        var file1 = ctx.request.files[0];
        var file2 = ctx.request.files[1];
        let readerStream1 = fs.createReadStream(file1.filepath);
        const target1 = path.join('app/public/upload/', file1.filename);
        let writerStream1 = fs.createWriteStream(target1);
        await readerStream1.pipe(writerStream1);

        let readerStream2 = fs.createReadStream(file2.filepath);
        const target2 = path.join('app/public/upload/', file2.filename);
        let writerStream2 = fs.createWriteStream(target2);
